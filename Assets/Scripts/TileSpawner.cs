...
public class TileSpawner : ITileSpawner
{
    ...
    private void Spawn(int x, int y, int levelIndex)
    {
        ...
        // JellyEffect'e do�ru scale'i ver
        JellyEffect jelly = tile.GetComponent<JellyEffect>();
        if (jelly != null)
        {
            jelly.SetOriginalScale(new Vector3(0.4f, 0.4f, 0.4f)); // Senin kulland���n scale
        }
    ...
    private System.Collections.IEnumerator ImprovedSpawnAnimation(Tile tile)
    {
        ...
        Vector3 targetScale = new Vector3(0.4f, 0.4f, 0.4f); // Senin scale de�erin
